...
ones that read sent. [CONTRIBUTING.md](CONTRIBUTING.md#docs) covers how.
...
pattern, since a global limit "will unnecessarily restrict requests to other
...
- [docs/errors.md](docs/errors.md) — what each failure reports and why
...
- [Per-endpoint rate limiting](https://copdips.com/2023/01/python-aiohttp-rate-limit.html)
  — the semaphore-per-endpoint pattern `MAX_CONCURRENT` uses.

## The layer above

This layer caches bytes. Every parser that reads through it caches what it
parsed out of them, on its own budget and its own idle timeout; whether a read
reaches this layer at all depends on that cache:
