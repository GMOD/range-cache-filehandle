...
/**
 * `expect(bytes).toEqual(other)` walks a typed array element by element through
 * the structural-equality machinery, which on the 1.25 MB ranges below costs
 * seconds per assertion and timed the suite out. Compare the bytes directly and
 * hand the first difference to `expect` only when there is one, so a failure
 * still reads as a failure rather than as a wall of numbers.
 */
function expectBytes(actual: Uint8Array, expected: Uint8Array) {
  expect(actual.length).toBe(expected.length)
  for (let i = 0; i < expected.length; i++) {
    if (actual[i] !== expected[i]) {
      expect({ offset: i, byte: actual[i] }).toEqual({
        offset: i,
        byte: expected[i],
      })
      return
...
/** the URL a mock fetch was called with, whichever of the three forms it took */
function requestedUrl(url: string | URL | Request) {
...
describe('RemoteFileWithRangeCache', () => {
  test('returns correct bytes for a small range within one chunk', async () => {
    ...
    expectBytes(result, slice(100, 199))
  ...
  test('returns correct bytes spanning two chunks', async () => {
    ...
    expectBytes(result, slice(start, end))
    expect(calls).toHaveLength(1)
  })
  ...
  test('gap detection fetches only missing middle chunks', async () => {
    ...
    expectBytes(result, slice(0, CHUNK * 4 - 1))
  ...
  test('evicting a needed chunk mid-fetch still assembles correct bytes', async () => {
    ...
    const mockFetch = async (
      ...
      if (m) {
        const start = Number(m[1])
        const end = Math.min(Number(m[2]), FILE_SIZE - 1)
        if (requestedUrl(url) === mainUrl && start >= CHUNK) {
          await chunk1Gate
        }
        return new Response(slice(start, end), { status: 206 })
      }
  ...
  test('known file size clamps a read extending past EOF', async () => {
    ...
    expectBytes(result, slice(start, FILE_SIZE - 1))
...
describe('RemoteFileWithRangeCache response deadline', () => {
  ...
  test("a caller's abort wins over a deadline still pending", async () => {
    ...
    await flush()
    vi.advanceTimersByTime(RESPONSE_TIMEOUT - 1000)
...
describe('RemoteFileWithRangeCache network-level failures', () => {
  ...
  test('an offline browser is told so instead of being told about CORS', async () => {
    // an own property shadowing whatever the environment provides, since node
    // supplies a `navigator` with no `onLine` on it at all
    ...
    try {
      ...
      // @ts-expect-error removing the shadow uncovers the environment's own
      delete navigator.onLine
    }
  })

  // Regression: `!navigator.onLine` reported every failure here as an offline
  // browser, because node's `navigator` has no such property. The CORS triage
  // is the message that is nearly always right for a genome file on someone
  // else's server, and it was unreachable outside a browser.
  test('an environment with no onLine at all still gets the CORS triage', async () => {
    ...
    expect(error).toMatch(/Access-Control-Allow-Origin/)
    expect(error).not.toMatch(/no network connection/)
  })
...
describe('RemoteFileWithRangeCache idle sweep', () => {
  ...
  test('a chunk nothing reads again is dropped, and re-fetched next time', async () => {
    ...
    vi.advanceTimersByTime(IDLE + SWEEP_INTERVAL)
  ...
  test('a chunk that keeps being read is never dropped', async () => {
    ...
    for (let i = 0; i < 8; i++) {
      vi.advanceTimersByTime(IDLE_TIMEOUT - SWEEP_INTERVAL)
      await fetchRange(file, 0, 99)
    }
    expect(calls).toHaveLength(1)
  })
